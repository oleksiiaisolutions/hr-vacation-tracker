...
async function fetchEmployees() {
    try {
        const { data, error } = await supabase
            ...
            `)
            .order('created_at', { ascending: true }); // Order mainly for stable list

        if (error) throw error;

        // Map Supabase snake_case to camelCase if needed, but schema uses:
        // name, birthday, start_date (needs map), requests

        _employees = data.map(e => ({
            id: e.id,
            name: e.name,
            birthday: e.birthday,
            startDate: e.start_date, // Map start_date to startDate
            requests: e.requests || []
        }));
...
async function addRequest(employeeId, request) {
    // request object from app: { date: 'YYYY-MM-DD', days: 2 }
    // Schema: employee_id, date, days
    ...
    if (error) {
        ...
        alert("Failed to add vacation request: " + error.message);
    }
}
...
function showEmptyState() {
    ...
    if (vacationForm) vacationForm.style.display = 'none'; // Hide form if no emp
...
function renderEmployeeList() {
    ...
    employees.forEach(emp => {
        ...
        // Avatar Initials
        const initials = emp.name.length > 0 ? emp.name.split(' ').map(n => n[0]).join('').substring(0, 2) : '??';
...
function refreshView() {
    ...
    if (!emp) {
        // Maybe deleted
        if (employees.length > 0) selectEmployee(employees[0].id);
        else showEmptyState();
        return;
    }

    if (deleteEmployeeBtn) deleteEmployeeBtn.style.display = 'block';
    if (vacationForm) vacationForm.style.display = 'flex'; // Show form
